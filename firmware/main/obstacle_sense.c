 ...
 * obstacle_sense.c
 *
 * Obstacle sensor interface with driver abstraction and swarm-wide scan fusion.
 *
 * Architecture:
 *   - A registered sensor_driver_fn is polled at OBSTACLE_SCAN_INTERVAL_MS (10 Hz)
 *   - Local scans are broadcast to the swarm via MSG_TYPE_OBSTACLE at 2 Hz
 *   - Remote scans from peer nodes are stored and expire after OBSTACLE_SCAN_TIMEOUT_MS
 *   - All public query APIs (get_closest, path_clear, get_all_scans) operate across
 *     both local and remote scans, giving every node a swarm-wide obstacle picture
 *   - Mock driver is active by default for testing without physical hardware
 */
...
static void broadcast_scan(const obstacle_scan_t *scan)
{
    ...
    // Build payload
    uint8_t payload[sizeof(obstacle_payload_t) + OBSTACLE_MAX_DETECTIONS * sizeof(obstacle_detection_packed_t)];
    ...
    header.flags = 0;
    header.payload_len = (uint8_t)payload_size;

    // Combine and send
    uint8_t packet[sizeof(swarm_header_t) + sizeof(obstacle_payload_t) + OBSTACLE_MAX_DETECTIONS * sizeof(obstacle_detection_packed_t)];
    memcpy(packet, &header, sizeof(swarm_header_t));
    memcpy(packet + sizeof(swarm_header_t), payload, payload_size);

    uint8_t broadcast_mac[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    swarm_transport_send(broadcast_mac, packet, sizeof(swarm_header_t) + payload_size);
...
{
    if (len < (int)sizeof(obstacle_payload_t)) {
    ...
    int expected_size = (int)sizeof(obstacle_payload_t) +
                        (hdr.num_detections * (int)sizeof(obstacle_detection_packed_t));
...
static void obstacle_recv_handler(const uint8_t *src_mac, const uint8_t *data,
                                  int len, int8_t rssi)
{
    if (len < (int)sizeof(swarm_header_t)) {
        return;
    }
    ...
    int payload_len = len - (int)sizeof(swarm_header_t);
...
static int cmd_mock_add(int argc, char **argv)
{
    ...
    int16_t range = (int16_t)atoi(argv[1]);
    int16_t bearing = (int16_t)atoi(argv[2]);
    int16_t velocity = (argc >= 4) ? (int16_t)atoi(argv[3]) : 0;
...
static int cmd_path_check(int argc, char **argv)
{
    ...
    int16_t bearing = (int16_t)atoi(argv[1]);
    int16_t corridor = (argc >= 3) ? (int16_t)atoi(argv[2]) : 30;
    int16_t range = (argc >= 4) ? (int16_t)atoi(argv[3]) : 3000;
