 ...
 *
 * Each node runs a local scan loop at 10 Hz using a registered sensor driver,
 * then broadcasts its scan to the swarm at 2 Hz via MSG_TYPE_OBSTACLE. All
 * nodes store remote scans from peers and expose a unified view of obstacles
 * across the entire swarm through a single API.
 *
 * The system ships with a mock driver enabled by default, allowing full swarm
 * testing without physical sensors attached. To use real hardware, implement
 * a function matching sensor_driver_fn and call obstacle_sense_register_driver()
 * before obstacle_sense_start():
 *
 *   bool my_driver(obstacle_scan_t *scan) {
 *       // Read hardware, populate scan->detections[], set scan->num_detections
 *       return true;
 *   }
 *   obstacle_sense_register_driver(my_driver);
 *
 * The a02yyuw module provides a ready-made driver for the DFRobot A02YYUW
 * waterproof ultrasonic sensor: obstacle_sense_register_driver(a02yyuw_scan_driver).
 */
